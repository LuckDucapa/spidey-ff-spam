...
class FF_CLIENT(threading.Thread):
    ...
    def connect(self, tok, key, iv, whisper_ip, whisper_port, online_ip, online_port):
        ...
        logging.info("Bot Fully Connected! Send /s7 <id> in game now.")
        ...
        while True:
            try:
                ...
                # Listen for Chat Message (Header 1200)
                if "1200" == hex_data[0:4]: 
                    try:
                        ...
                        if b'/s7' in data:
                            logging.info(f"[DEBUG] Found '/s7' command! Sender UID: {uid}")
                            try:
                                # Extract target
                                parts = re.split(b'/s7', data)[1].decode(errors='ignore').strip()
                                target_match = re.search(r'\d+', parts)
                                
                                if target_match:
                                    target = target_match.group()
                                    logging.info(f"[DEBUG] Target extracted: {target}")
                                    
                                    if uid:
                                        clients.send(self.GenResponsMsg(f"[C][B][00FF00]Spamming {target}...", uid))
                                    ...
                                    packet = self.request_join_squad(target)
                                    for i in range(1, 36):
                                        socket_client.send(packet)
                                        if i % 10 == 0: logging.info(f"Sent {i} invites...")
                                        time.sleep(0.05)
                                        
                                    if uid:
                                        clients.send(self.GenResponsMsg("[C][B]Done!", uid))
                                    socket_client.send(self.leave_s())
                                else:
                                    logging.error("Command found but no ID detected.")
                            except Exception as e:
                                logging.error(f"Error executing /s7: {e}")
                    except: pass
