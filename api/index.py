...
# Import your local files (Ensure these are in the root directory or adjust path)
# On Vercel, we need to add the parent directory to sys.path to find modules
...
try:
    ...
    # Fallback for local testing vs Vercel paths
    pass
...
try:
    with open('bot.txt', 'r') as file:
        BOT_CREDS = json.load(file)
        BOT_UID = list(BOT_CREDS.keys())[0]
        BOT_PASS = BOT_CREDS[BOT_UID]
except:
    BOT_UID = "YOUR_BACKUP_UID" 
    BOT_PASS = "YOUR_BACKUP_PASS"
...
class FF_ONE_TIME_RUNNER:
    ...
    def request_join_squad(self, idplayer):
        # 4096 = Moderator Badge Logic (Type 1)
        same_value = 4096 
        fields = {
            1: 33, 
            2: {
    ...
    def execute(self, target_uid):
        ...
        if 'access_token' not in r: return False, "Guest Login Failed"
        ...
        if not parsed: return False, "Major Login Failed"
